...
const HobbyCard = ({ category, index }) => (
  <div
    data-aos="fade-up"
  ...
  >
    <div>
      <div className="flex justify-between items-start mb-6">
        <span className="text-4xl p-3 bg-white/5 rounded-2xl group-hover:bg-[#c9ada7]/10 group-hover:scale-110 transition-all duration-300">
          {category.icon}
        </span>
      ...
      <h3 className="text-white text-xl md:text-2xl font-black mb-3 tracking-tight group-hover:text-[#c9ada7] transition-colors">
        {category.title}
      </h3>
...
const Hobbies = () => {
  return (
    <section
      id="hobbies"
      className="bg-[#1a1a2e] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-serif bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]"
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          {hobbies.categories.map((category, index) => (
            <HobbyCard
              key={category.title}
              category={category}
              index={index}
